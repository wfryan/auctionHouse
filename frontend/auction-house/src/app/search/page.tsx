...
export default function Search() {
    ...
    const searchFunc = async () => {
        ...
        try {
            ...
            const response = await instance.post('/items/searchItems', JSON.stringify(body))
    ...
    function displayItem(item: AuctionItem): boolean {
        ...
        if (getToken() != null) {
            if (item.status == 'bought' || item.status == 'archived' /*further time checks required */) {
                return true;
            } else {
                return false;
            }
    ...
    return (
        <div className="min-h-screen py-8 px-4">
            ...
            {/* Search bar section */}
            <div className="mt-6 px-4 flex flex-col sm:flex-row items-center gap-4">
                <input
                ...
                />
                <button
                    onClick={() => searchFunc()} disabled={isButtonDisabled}
            ...
            <div className="mt-6 px-5">
                <div className="max-w-md">
                    {auctions.length > 0 &&
                        // modify to include bought if you are signed in
                        auctions.filter(item => (displayItem(item)))
                            .map((item) => (
                                <div key={item.auction_item_id} className="mb-6 bg-gray-100 rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300">
                                    <div className="p-4" onClick={() => setStorage(item.auction_item_id)}>
